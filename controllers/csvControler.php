...
class csvControler
{
    ...
    public static function descargarVentaPdf($req, $res, $args)
    {
        $tabla = $args['baseDatos'];
        if (
            $tabla == 'ventaarmas' ||
            ...
            $objAccesoDatos = AccesoDatos::obtenerInstancia();
            $consulta = $objAccesoDatos->prepararConsulta("SELECT * FROM  $tabla WHERE fecha > DATE_SUB(NOW(), INTERVAL 1 MONTH) ORDER BY fecha DESC");  //WHERE fecha > DATE_SUB(NOW(), INTERVAL 1 MONTH) ORDER BY fecha DESC
            // $consulta->bindValue(':tabla', $args['db']);
            $consulta->execute();
            $resultados = $consulta->fetchAll(PDO::FETCH_ASSOC);
            ...
            $res = $res
                ->withHeader('Content-Type', 'text/csv')
                ->withHeader('Content-Disposition', 'attachment; filename="' . $tabla . '.pdf"')
                ->withBody(new \Slim\Psr7\Stream(fopen('php://temp', 'r+')));
